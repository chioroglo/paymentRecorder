...
public class AgentService : BaseEntityService<Agent>, IAgentService
{
    ...
    {
        var entityInDatabase = await _agentsWithAccounts.Include(e => e.Accounts)
                                   .FirstOrDefaultAsync(e => e.FiscalCode == fiscalCode, cancellationToken) ??
                               throw new EntityValidationException(
                                   EntityWasNotFoundBecause<Agent>($"with FiscalCode: {fiscalCode} does not exist"));
    ...
    public async Task<Agent> Add(Agent entity, CancellationToken cancellationToken)
    {
        ...
        {
            throw new EntityValidationException(
                EntityCannotBeCreatedBecause<Agent>(
                    $"has fiscal code, that is already assigned to another {nameof(Agent)}"));
        }
    ...
    {
        var entity =
            await _db.Agents.Include(e => e.Accounts).FirstOrDefaultAsync(e => e.Id == id, cancellationToken) ??
    ...
    public async Task<Agent> UpdateAsync(Agent entity, CancellationToken cancellationToken)
    {
        var entityInDatabase = await _db.Agents.FirstOrDefaultAsync(e => e.Id == entity.Id, cancellationToken) ??
                               throw new EntityValidationException(
                                   EntityWasNotFoundBecause<Agent>($"of ID:{entity.Id} does not exist"));
